...
async function main() {
  ...
  const result = await payload.find({
    collection: 'posts', where: { id: { equals: 19 } }, limit: 1, overrideAccess: true,
  })
  const post = result.docs[0] as any
  if (!post) throw new Error('Post 19 not found')
  console.log('Found post:', post.slug)

  const REMOVE_MEDIA_ID = 33
  ...
  await payload.update({
    collection: 'posts', id: 19, data: { content: updatedContent },
